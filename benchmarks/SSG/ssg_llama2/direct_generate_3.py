...
def main():
    ...
    for idx in tqdm(range(len(inference_data_tokenized)), desc="Generating outputs"):
        ...
        try:
            ...
            print(f"Error generating text for index {idx}: {e}")
            generated_text = ""
        ...
        # 打印生成结果（调试）
        print(f"Generated {idx+1}/{len(inference_data_tokenized)}: {generated_text}\n")

    # 保存生成的内容到文件
    output_file = pjoin(args.output_dir, 'l2p_paper_generated_output.json')
